#ifndef OBJECTBASE_HPP__
#define OBJECTBASE_HPP__
...
private:
    template <typename Func> static void DisplayAllObjects(Func displayAndAnimateFunc) {
        for (int layer = MAX_LAYERS - 1; layer >= 0; layer--) {
            for (auto &obj : GetObjects(static_cast<LayerID>(layer))) {
                obj->m_currentFrame = displayAndAnimateFunc(obj->m_imageID, obj->m_animID, obj->m_x,
